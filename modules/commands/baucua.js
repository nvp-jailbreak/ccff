var request = require("request");const { readdirSync, readFileSync, writeFileSync, existsSync, copySync, createWriteStream, createReadStream } = require("fs-extra");
    module.exports.config = {
        ...
        version: "1.0.0",
        hasPermssion: 0,
        credits: "Horizon Mod nhẹ by NVP",
...
var full = [];
        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    if (!args[0] || !isNaN(args[0])) return api.sendMessage("[NVP] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]",event.threadID, event.messageID);
                    if (isNaN(moneyBet) || moneyBet <= 0) return api.sendMessage("[NVP] => Số tiền đặt cược không được để trống hoặc là số tiền âm", event.threadID, event.messageID);
                if (moneyBet > moneyUser) return api.sendMessage("[NVP] => Số tiền bạn đặt lớn hơn số dư của bạn!", event.threadID, event.messageID);
            if (moneyBet < 1000) return api.sendMessage("[NVP] => Số tiền đặt không được dưới 1000 đô!", event.threadID, event.messageID);
                ...
                switch (args[0]) {
                    ...
                    case "tôm":
                        case "Tôm": itemm = "tom";
                                icon = '🦞';
                            break;
                                default: return api.sendMessage("[NVP] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]",event.threadID, event.messageID);
                }      
                await get(number[0],number[1],number[2]);
            api.sendMessage({body:"Đ𝐮́𝐧𝐠 𝐥𝐚̀ 𝐭𝐡𝐮̛́ 𝐧𝐠𝐡𝐢𝐞̣̂𝐧 𝐧𝐠𝐚̣̂𝐩 𝐜𝐨̛̀ 𝐛𝐚̣𝐜, 𝐫𝐨̂̀𝐢 𝐦𝐚𝐢 𝐬𝐚𝐮 𝐧𝐚̀𝐲 𝐦𝐚̀𝐲 𝐬𝐞̃ 𝐥𝐚̀𝐦 𝐠𝐢̀ 𝐜𝐡𝐨 đ𝐚̂́𝐭 𝐧𝐮̛𝐨̛́𝐜 🌺\nĐ𝐚𝐧𝐠 𝐥𝐚̆́𝐜 𝐜𝐡𝐨̛̀ 𝐭𝐚𝐨 𝟏 𝐭𝐢́ 𝐧𝐡𝐞́ 😘",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            else return api.sendMessage("[NVP] => Lỗi ! Code : XX1N",event.threadID,event.messageID);
        } else  {
